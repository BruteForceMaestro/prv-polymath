...
5. Sub-researcher Agent (via assign_researcher) - use if needing to "divide and conquer", assigning more
...
class ResearcherWork(AgentWork):
    ...
    async def assign_subresearcher(self, task: str):
        return await assign_researcher(task, self.recursive_depth+1)
...
async def assign_researcher(task: str, recursive_depth=0):
    ...
    researcher_tools : list[ToolType] = [
        ...
        search_in_graph, research_literature, verify_lean, verify_numeric, tracker.assign_subresearcher,
